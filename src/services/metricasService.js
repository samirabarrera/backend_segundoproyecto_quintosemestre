...
// Reglas para determinar status_code, criticidad y mensaje descriptivo
 ...
 * Tras el INSERT emite los eventos Socket.io correspondientes.
 * @param {{ nodoId: string, vatiosGenerados: number, voltaje: number }}
...
const saveMetric = async ({ nodoId, vatiosGenerados, voltaje }) => {
  ...
  const mensaje = generarMensaje(criticidad, vatiosGenerados, voltaje, nodoNombre);
  ...
  // getIO() lanza error si Socket.io no está inicializado (jamás llegará aquí)
  const io = getIO();
...
/**
 * Retorna las métricas de un nodo en los últimos N minutos.
